...
def hash_position(seed: int, bloom_filter_length: int, hash_number: int):
    x = int(seed) % (2**64)
    rng = np.random.default_rng(x)
    H = rng.integers(0, bloom_filter_length, size=hash_number)
    return tuple(H.tolist())

def bloom_filter(flat_img: np.ndarray, bloom_filter_length: int, hash_number: int):   
    bf_list = []
    for data in flat_img:
        bf = np.zeros(k, dtype=np.int8)
        seed = img_processed(data)
        for s in seed:
            bf[list(hash_position(s, bloom_filter_length, hash_number))] = 1
        bf_list.append(np.packbits(bf.astype(np.int8)))
    return bf_list
...
def load_data(data_set, PI, L=None):
    """
    data_set:
...
def compute_hash_number_and_BF_length(fp, m):
...
def main():
    global check
    #CWALDPを実施しないときは PI=1,L=256
    ap = argparse.ArgumentParser()
    ap.add_argument("--fp", type=int, default=None,help="rate of false positive")
    ap.add_argument("--hash_number", type=int, default=None,help="the number of hash function")
    ap.add_argument("--npz_path", type=str, default=None)
    ap.add_argument("--data_set", type=str, default=None)
    ap.add_argument("--PI", type=float, default=1)
    ap.add_argument("--L", type=int, default=256)  # ★ 追加：L 段階量子化
    args = ap.parse_args()
    ...
    if args.npz_path is not None:
        ...
        # save directory path
        # L を load_data に渡す
        X, y = load_data(args.data_set, args.PI, args.L)
        #784×(2*近傍数＋1)
        m = X.shape[1]
        bloom_filter_length, hash_number = compute_hash_number_and_BF_length(args.fp, m)
        
        if args.hash_number!=None:
         hash_number=args.hash_number
        print("[INFO] Making BF...")
        X = bloom_filter(X, bloom_filter_length, hash_number)
        meta = dict(fp=args.fp, bf_legnth=bloom_filter_length, k=hash_number, dataset=args.dataset)
        os.makedirs(args.npz_path, exist_ok=True)
        output_path = os.path.join(args.npz_path,f"fmnist_rappor_fp{args.fp}_bf_length{bloom_filter_length}_hash_number{hash_number}_PI{args.PI}_L{args.L}.npz")
        ...
        print(f"[SAVED] fmnist_rappor_fp{args.fp}_bloomfiter_length{bloom_filter_length}_hash_number{hash_number}_PI{args.PI}_L{args.L}.npz")
