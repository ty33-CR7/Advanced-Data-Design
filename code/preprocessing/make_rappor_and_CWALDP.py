...
def main():
    ...
    ap.add_argument("--fp", type=float, default=None,help="rate of false positive")
    ...
    if args.npz_path is not None:
        ...
        meta = dict(fp=args.fp, l=bloom_filter_length, k=hash_number, dataset=args.dataset)
        os.makedirs(args.npz_path, exist_ok=True)
        output_path = os.path.join(args.npz_path,f"fmnist_rappor_fp{args.fp}_bloomfiter_length{bloom_filter_length}_hash_number{hash_number}_PI{args.PI}_L{args.L}.npz")
