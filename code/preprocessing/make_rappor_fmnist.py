...
def main():
    ...
    if args.label_cluster != 0:
        base_ep = args.label_ep
    if base_ep > 0.0:
        y = y.reshape(-1)
        label_domain = list(10)
        label_epsilon = compute_label_epsilon(base_ep, args.label_cluster)
        print(f"[INFO] Apply RR to labels: base_ep={base_ep}, label=epsilon{label_epsilon}, cluster={args.label_cluster}")
        
        y_noisy = grr_array(y, label_epsilon, label_domain, args.seed)
        y = y_noisy.astype(np.int64)
    else:
        print("[INFO] [ERROR] label_cluster != 0 but base_e=0")
    
    if args.bfonly:
        print(f"[INFO] bfonly=True")
        out_name = os.path.join(
            args.outdir,
            f"fmnist_rappor_bfonly_m{args.m}_k{args.k}_h{args.h}.npz"
        )
    else:
        ...
        out_name = os.path.join(
            args.outdir,
            f"fmnist_rappor_k{args.k}_h{args.h}_m{args.m}_f{f}_q{args.q}_p{args.p}.npz"
        )
        ...
        if args.permonly:
            ...
            if args.m:
                for i in range(len(X)):
                    if i % 1000 == 0:
                        print(f"[INFO] {i}/70000 are finished")
