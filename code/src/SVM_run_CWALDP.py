...
def train_SVM(X_train_noise, X_test_noise, y_train_noise, y_test):
    """
    SVM（サポートベクターマシン）を用いて学習と評価を行う
    """
    # 1. モデルの定義
    # サンプル数が多い場合は LinearSVC の方が圧倒的に高速です。
    # 非線形分離が必要な場合は SVC(kernel='rbf') を使用しますが、時間がかかります。
    model = LinearSVC(
        C=1.0, 
        tol=0.001,
        class_weight=None,
        random_state=42, 
        max_iter=1000, 
        dual=False # サンプル数 > 特徴量数の場合は False 推奨
    )
    ...
    # 3. 推論と精度評価
    test_acc = model.score(X_test_noise, y_test)
    test_loss = 0.0  # SVMにlossの概念はないため0固定

    # 4. モデルパラメータを取得
    model_params = json.dumps(model.get_params(), indent=2)
    
    return test_loss, test_acc, model_params
...
def train_RF(X_train_noise, X_test_noise, y_train_noise, y_test):
    """
    ...
    # Kerasのlossに相当する値はRFでは直接得られないため 0.0 を返します
    test_loss = 0.0 
...
def waldp_time(original_path, output_path, epsilon, pixel, L,cluster_num, test_env, seeds,label_epsilon,data):
    ...
    for fid in range(1, 11):
        ...
        for seed in seeds:
            ...
            test_loss, test_acc, current_summary = train_SVM(
                        X_train_noise, 
                        X_test_noised, 
                        y_train_noise, 
                        y_test
...
if __name__ == "__main__":
    ...
    epsilons=[0,1*196/49,2*196/49,3*196/49]
    params = [(7*7,4,8,"UTS")] 
    for P, L,cluster_num,test_env in params:
        for eps in epsilons:
            ...
            output_path = f"../../experiments/{data}/CWALDP/SVM/RR_waldp_L{L}_PI{P}_C{cluster_num}_eps{eps}_env{test_env}_label_noise_{label_epsilon}_time.csv"
